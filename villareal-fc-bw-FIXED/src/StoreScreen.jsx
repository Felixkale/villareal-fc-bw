...
import { NAVY, GOLD, GOLD2, WHITE, MGRAY, RED, GREEN, LGRAY, Logo, Btn, SQUAD } from "./constants"
...
const StoreScreen=({goToAuth,fixtures,openMembership,session,profile})=>{
  ...
  // ── Tier-aware membership detection ────────────────────────────────────────
  const tier          = profile?.tier
  const isHoneyBadger = tier === "honey_badger"
  const isGlobalFan   = tier === "global_fan"
  const isMember      = isHoneyBadger || isGlobalFan
  const isPremiumFree = !isMember

  // Discount by tier
  const memberDisc =
    isHoneyBadger ? 10 :   // HB gets 10% off store
    isGlobalFan   ? 5  :   // Global Fan gets 5% off
    0

  const tierLabel =
    isHoneyBadger ? "🦡 HONEY BADGER" :
    isGlobalFan   ? "🌍 GLOBAL FAN"   :
    "FREE FAN"
  ...
  const applyPromo = () => {
    ...
    if(promo.memberOnly&&!isMember){
      setPromoMsg({ok:false,text:"This code is for members only."}); return
    }
    setPromoCode(code)
  ...
  const ticketPrice = (basePrice) => {
    if(isHoneyBadger) return Math.round(basePrice * 0.80)  // 20% off
    if(isGlobalFan)   return Math.round(basePrice * 0.90)  // 10% off
    return basePrice
  }
  ...
  /* ── HERO BANNER (isolated to prevent scroll reset) ── */
  const HeroBannerBlock = React.memo(() => {
    ...
    return (
      <div style={{position:"relative",overflow:"hidden",flexShrink:0}}>
        <div style={{background:b.bg,padding:"clamp(24px,6vw,36px) clamp(16px,4vw,20px) clamp(20px,5vw,28px)",
          transition:"background 0.6s",position:"relative",overflow:"hidden",
          minHeight:"clamp(160px,40vw,200px)",display:"flex",flexDirection:"column",justifyContent:"center"}}>
          ...
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
            fontSize:"clamp(22px,7vw,32px)",color:b.accent,lineHeight:1,letterSpacing:"0.04em"}}>{b.title}</div>
          <div style={{fontSize:"clamp(11px,3vw,13px)",color:"rgba(255,255,255,0.7)",marginTop:4,marginBottom:14}}>{b.sub}</div>
          <button onClick={()=>{setActiveCol(b.col);setShopView("collection")}}
            style={{alignSelf:"flex-start",background:b.accent,border:"none",borderRadius:8,
              padding:"8px 18px",minHeight:38,fontFamily:"'Barlow Condensed',sans-serif",
              fontWeight:900,fontSize:"clamp(11px,3vw,13px)",color:NAVY,cursor:"pointer",
              WebkitTapHighlightColor:"transparent",letterSpacing:"0.06em"}}>{b.cta}</button>
        </div>
  ...
  const HomeView = () => (
    <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",background:"#f5f6fa"}}>
      ...
      {session && isMember && (
        <div style={{margin:"12px 12px 0",background:`linear-gradient(135deg,${NAVY},#1a3060)`,
          borderRadius:12,padding:"12px 16px",
          display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
              fontSize:14,color:tierColor,letterSpacing:"0.06em"}}>{tierLabel}</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:2}}>
              {isHoneyBadger
                ? "10% off store · 20% off tickets · Free match entry"
                : "5% off store · 10% off tickets · Early access"}
            </div>
          </div>
          <div style={{background:tierColor,color:NAVY,fontFamily:"'Barlow Condensed',sans-serif",
      ...
      {/* ── UPGRADE BANNER (free/not logged in) ── */}
      {(!session || isPremiumFree) && (
        <div onClick={session ? openMembership : goToAuth}
          ...
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
              ...
              {session ? "🦡 UPGRADE — SAVE UP TO 10% ON STORE" : "🦡 JOIN TO UNLOCK MEMBER PRICES"}
            </div>
            <div style={{fontSize:11,color:"rgba(13,27,62,0.7)",marginTop:2}}>
              {session ? "Global Fan 5% · Honey Badger 10% · Plus ticket discounts" : "Log in or join to access member discounts"}
            </div>
          </div>
      ...
      <div style={{padding:"16px 12px 8px"}}>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
          fontSize:"clamp(16px,5vw,20px)",color:NAVY,marginBottom:12,letterSpacing:"0.04em"}}>
          COLLECTIONS
        </div>
          ...
          {ALL_PRODUCTS.filter((_,i)=>[0,3,6,10,12].includes(i)).map(p=>(
            <div key={p.id}
              ...
              <div style={{height:100,background:`linear-gradient(160deg,${NAVY},#1a3060)`,
                ...
                <div style={{position:"absolute",top:7,left:7,background:p.tagC,color:WHITE,
                  fontSize:8,fontWeight:900,padding:"2px 6px",borderRadius:3,
                  fontFamily:"'Barlow Condensed',sans-serif"}}>{p.tag}</div>
                {totalDisc>0&&(
                  <div style={{position:"absolute",top:7,right:7,background:RED,color:WHITE,
                    fontSize:8,fontWeight:900,padding:"2px 6px",borderRadius:3,
              ...
              <div style={{padding:"8px 10px 10px"}}>
                ...
                <div style={{display:"flex",alignItems:"baseline",gap:5}}>
                  <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                    fontSize:"clamp(14px,4vw,17px)",color:NAVY}}>P{discPrice(p.price)}</span>
                  {totalDisc>0&&(
                    <span style={{fontSize:10,color:MGRAY,textDecoration:"line-through"}}>P{p.price}</span>
                  )}
                </div>
                {isMember&&<div style={{fontSize:9,color:tierColor,fontWeight:700,marginTop:1}}>{tierLabel} price</div>}
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:col.bg,padding:"16px 14px 14px",flexShrink:0}}>
          ...
          {isMember&&(
            <div style={{marginTop:8,background:"rgba(0,0,0,0.3)",borderRadius:8,
              padding:"6px 12px",display:"inline-flex",alignItems:"center",gap:6}}>
              <span style={{fontSize:12,color:tierColor,fontWeight:800,
                fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>
                {tierLabel} · {memberDisc}% OFF ALL ITEMS
              </span>
            </div>
          )}
          <div style={{fontSize:12,color:"rgba(255,255,255,0.65)",marginTop:8,lineHeight:1.5}}>{col.desc}</div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:"12px",background:"#f5f6fa",WebkitOverflowScrolling:"touch"}}>
            ...
            {col.products.map(p=>(
              <div key={p.id}
                ...
                <div style={{height:110,background:`linear-gradient(160deg,${NAVY},#1a3060)`,
                  ...
                  <div style={{position:"absolute",top:8,left:8,background:p.tagC,color:WHITE,
                    fontSize:8,fontWeight:900,padding:"2px 6px",borderRadius:3,
                    fontFamily:"'Barlow Condensed',sans-serif"}}>{p.tag}</div>
                  {totalDisc>0&&(
                    <div style={{position:"absolute",bottom:8,right:8,background:RED,color:WHITE,
                      fontSize:8,fontWeight:900,padding:"2px 6px",borderRadius:3,
                      fontFamily:"'Barlow Condensed',sans-serif"}}>-{totalDisc}%</div>
                  )}
                </div>
                <div style={{padding:"10px 10px 12px"}}>
                  ...
                  {isMember&&<div style={{fontSize:9,color:tierColor,fontWeight:700,marginTop:2}}>{tierLabel} price</div>}
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:WHITE}}>
        ...
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
          <div style={{background:`linear-gradient(160deg,${NAVY},#1a3060)`,
            ...
            <div style={{position:"absolute",top:12,left:12,background:p.tagC,color:WHITE,
              fontSize:10,fontWeight:900,padding:"3px 9px",borderRadius:4,
              fontFamily:"'Barlow Condensed',sans-serif"}}>{p.tag}</div>
            {totalDisc>0&&(
              <div style={{position:"absolute",top:12,right:12,background:RED,color:WHITE,
                fontSize:10,fontWeight:900,padding:"3px 9px",borderRadius:4,
          ...
          <div style={{padding:"16px 14px"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
              fontSize:"clamp(18px,5vw,22px)",color:NAVY,lineHeight:1,marginBottom:8}}>{p.name}</div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
              {isMember&&(
                <div style={{background:`${tierColor}22`,border:`1px solid ${tierColor}`,
                  borderRadius:6,padding:"3px 9px",fontSize:11,color:NAVY,fontWeight:700}}>
                  {tierLabel} -{memberDisc}%
                </div>
              )}
              {promoMsg?.ok&&(
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              ...
              <button onClick={()=>setShowGuide(s=>!s)}
                style={{background:"none",border:"none",cursor:"pointer",
                  fontSize:11,color:NAVY,fontWeight:700,textDecoration:"underline"}}>Size Guide</button>
            </div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
              {p.sizes.map(sz=>(
                <button key={sz} onClick={()=>setSelSize(sz)} style={{
                  minWidth:"clamp(38px,10vw,48px)",height:"clamp(38px,10vw,48px)",borderRadius:9,
                  border:`2px solid ${selSize===sz?NAVY:"#e5e7eb"}`,
                  background:selSize===sz?NAVY:WHITE,color:selSize===sz?WHITE:NAVY,
                  fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
            ...
            {p.tag!=="KIDS"&&p.tag!=="FAN"&&(
              <div onClick={()=>setShopView("customize")}
                style={{background:LGRAY,borderRadius:12,padding:"12px 14px",marginBottom:14,
            ...
            <div style={{fontSize:12,color:MGRAY,lineHeight:1.6,marginBottom:16}}>
              {COLLECTIONS.find(c=>c.id===p.collection)?.desc} Season: {p.season}.
            </div>
            ...
            <button
              onClick={()=>{
                ...
                handleAddToCart({...p,price:q},selSize,selVariant,selQuality,
                  {name:customMode==="name"?customName:"",player:customMode==="player"?selPlayer:""})
              }}
              disabled={!selSize}
              ...
              {selSize ? `ADD TO CART — P${discPrice(selQuality==="Match"?p.price+100:p.price)}` : "SELECT A SIZE TO CONTINUE"}
            </button>

            {/* Promo code */}
            <div style={{marginTop:16,background:LGRAY,borderRadius:10,padding:"12px 14px"}}>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={applyPromo}
                  style={{background:NAVY,border:"none",borderRadius:8,padding:"9px 14px",
                    ...
                    fontSize:12,cursor:"pointer",flexShrink:0,WebkitTapHighlightColor:"transparent"}}>
                  APPLY
                </button>
              </div>
              {promoMsg&&(
  ...
  const CustomizeView = () => {
    ...
    const [cMode,    setCMode]    = useState("player")
    ...
    const finalPrice = kit ? discPrice(kit.price + 100) : 0
    ...
    const doAdd = () => {
      ...
      handleAddToCart(
        ...
        {name:cMode==="name"?`${cName} ${cNum}`.trim():"",player:cMode==="player"?cPlayer:""}
      )
      setAdded(true)
    ...
    return (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:"linear-gradient(135deg,#7c3aed,#4c1d95)",padding:"14px 16px",flexShrink:0}}>
          <button onClick={()=>setShopView("home")}
            style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,
              padding:"6px 12px",color:WHITE,fontSize:13,cursor:"pointer",
              fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,marginBottom:10,
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:"rgba(255,255,255,0.6)",marginTop:2}}>Name · Number · Player · +P100</div>
            </div>
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",padding:"14px",background:"#f5f6fa"}}>
          {/* Live preview */}
          <div style={{background:"linear-gradient(135deg,#7c3aed,#4c1d95)",borderRadius:16,
            padding:"20px",marginBottom:16,textAlign:"center",position:"relative",overflow:"hidden"}}>
            ...
            {(cPlayer||cName)&&(
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                fontSize:"clamp(20px,6vw,28px)",color:"#fde68a",letterSpacing:"0.1em",lineHeight:1,marginBottom:4}}>
            ...
            {(cNum||cMode==="player"&&cPlayer)&&(
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                ...
                {cMode==="player"?(PLAYERS.find(p=>p.name===cPlayer)?.num||"#?"):(cNum||"#?")}
              </div>
            )}
            {(!cPlayer&&!cName)&&(
            ...
            {kit&&<div style={{marginTop:8,fontSize:11,color:"rgba(255,255,255,0.7)"}}>{kit.label}</div>}
          </div>

          {/* Step 1 */}
          <div style={{background:WHITE,borderRadius:12,padding:"14px",marginBottom:12,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:11,color:MGRAY,letterSpacing:"0.08em",marginBottom:10}}>1 · CHOOSE KIT</div>
              ...
              {KITS.map(k=>(
                <div key={k.id} onClick={()=>setCKit(k.id)}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
                      fontSize:12,color:NAVY,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{k.label}</div>
          ...
          {/* Step 2 */}
          <div style={{background:WHITE,borderRadius:12,padding:"14px",marginBottom:12,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:11,color:MGRAY,letterSpacing:"0.08em",marginBottom:10}}>2 · VARIANT & SIZE</div>
            <div style={{marginBottom:10}}>
              <div style={{fontSize:10,color:MGRAY,fontWeight:700,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>VARIANT</div>
              <div style={{display:"flex",gap:8}}>
                {["Men","Women","Junior"].map(v=>(
                  <button key={v} onClick={()=>setCVariant(v)} style={{
                    ...
                    background:cVariant===v?NAVY:WHITE,color:cVariant===v?WHITE:NAVY,
                    fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
            ...
            <div style={{fontSize:10,color:MGRAY,fontWeight:700,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>SIZE</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {(cVariant==="Junior"
                ...
                <button key={sz} onClick={()=>setCSize(sz)} style={{
                  ...
                  background:cSize===sz?"#7c3aed":WHITE,color:cSize===sz?WHITE:NAVY,
                  fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
                  fontSize:12,cursor:"pointer",WebkitTapHighlightColor:"transparent",
                }}>{sz}</button>
              ))}
          ...
          {/* Step 3 */}
          <div style={{background:WHITE,borderRadius:12,padding:"14px",marginBottom:12,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:11,color:MGRAY,letterSpacing:"0.08em",marginBottom:10}}>3 · PERSONALISE</div>
            <div style={{display:"flex",gap:6,marginBottom:12,background:LGRAY,borderRadius:10,padding:4}}>
            ...
            {cMode==="player"?(
              <>
                <div style={{fontSize:10,color:MGRAY,fontWeight:700,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>SELECT PLAYER</div>
                  ...
                  {PLAYERS.map(p=>(
                    <div key={p.id} onClick={()=>setCPlayer(p.name)}
                      ...
                      <div style={{width:32,height:32,borderRadius:8,
                        ...
                        <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:11,color:GOLD}}>{p.num}</span>
                      </div>
                      <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,color:NAVY,flex:1}}>{p.name}</span>
              ...
              <>
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:10,color:MGRAY,fontWeight:700,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>YOUR NAME (max 12 chars)</div>
                ...
                <div>
                  <div style={{fontSize:10,color:MGRAY,fontWeight:700,marginBottom:6,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.06em"}}>SQUAD NUMBER</div>
                  <input placeholder="e.g. #9" value={cNum}
                    ...
                    style={{width:"100%",padding:"11px 12px",borderRadius:8,
                      ...
                      fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                      letterSpacing:"0.1em",outline:"none",boxSizing:"border-box",textAlign:"center"}}/>
                </div>
              </>
            )}
          </div>
          ...
          {kit&&(
            <div style={{background:LGRAY,borderRadius:10,padding:"12px 14px",marginBottom:14,
              display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontSize:12,color:MGRAY}}>{kit.label} + personalisation</div>
                {totalDisc>0&&<div style={{fontSize:11,color:GREEN,fontWeight:700}}>{totalDisc}% discount applied</div>}
              </div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:22,color:NAVY}}>P{finalPrice}</div>
            </div>
          )}

          <button onClick={doAdd} disabled={!cKit||!cSize}
            style={{width:"100%",padding:"16px",minHeight:54,
              background:!cKit||!cSize?"#e5e7eb":"#7c3aed",border:"none",borderRadius:12,
              ...
              transition:"all 0.15s",letterSpacing:"0.04em"}}>
            {added?"✓ ADDED TO CART!":!cKit?"SELECT A KIT FIRST":!cSize?"SELECT A SIZE":`ADD TO CART — P${finalPrice}`}
          </button>
          <div style={{height:20}}/>
        </div>
      </div>
  ...
  const CartView = () => (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:WHITE}}>
      <div style={{background:NAVY,padding:"14px 16px",flexShrink:0,
        ...
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:16,color:WHITE}}>CART ({cartQty})</div>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"12px 14px",WebkitOverflowScrolling:"touch",background:"#f5f6fa"}}>
        {cart.length===0?(
          <div style={{textAlign:"center",padding:"48px 20px"}}>
            <div style={{fontSize:52,marginBottom:12}}>🛒</div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:20,color:NAVY,marginBottom:6}}>YOUR CART IS EMPTY</div>
            ...
            <button onClick={()=>setShopView("home")}
              style={{background:NAVY,border:"none",borderRadius:10,padding:"12px 24px",
                fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:14,color:WHITE,cursor:"pointer"}}>
              BROWSE STORE
            </button>
          </div>
        ):(
          <>
            {totalDisc>0&&(
              <div style={{background:`${GREEN}18`,border:`1px solid ${GREEN}44`,borderRadius:10,
            ...
            {cart.map((item,i)=>(
              <div key={item.cartId} style={{background:WHITE,borderRadius:12,marginBottom:10,
                ...
                <div style={{display:"flex",gap:12,padding:"12px 14px"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,
                      fontSize:14,color:NAVY,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.name}</div>
                    <div style={{fontSize:11,color:MGRAY,marginTop:2}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:6}}>
                      <div>
                        <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:16,color:NAVY}}>P{discPrice(item.price)}</span>
                        {item.price!==discPrice(item.price)&&(
                          <span style={{fontSize:10,color:MGRAY,textDecoration:"line-through",marginLeft:5}}>P{item.price}</span>
                        )}
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <button onClick={()=>setCart(prev=>prev.map((c,j)=>j===i?{...c,qty:Math.max(1,c.qty-1)}:c))}
                          style={{width:28,height:28,borderRadius:"50%",background:LGRAY,border:"none",cursor:"pointer",fontSize:16,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center"}}>-</button>
                        ...
                        <button onClick={()=>setCart(prev=>prev.filter((_,j)=>j!==i))}
                          style={{background:"#fef2f2",border:"none",borderRadius:6,padding:"4px 8px",fontSize:11,color:RED,fontWeight:700,cursor:"pointer"}}>Remove</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div style={{background:WHITE,borderRadius:12,padding:"12px 14px",marginBottom:10,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:11,color:NAVY,letterSpacing:"0.08em",marginBottom:8}}>🎟 PROMO CODE</div>
              <div style={{display:"flex",gap:8}}>
                <input placeholder="Enter code" value={promoInput}
                  onChange={e=>setPromoInput(e.target.value.toUpperCase())}
                  style={{flex:1,padding:"9px 12px",borderRadius:8,
                    border:`1.5px solid ${promoMsg?.ok?GREEN:promoMsg?.ok===false?RED:"#ddd"}`,
                    fontSize:13,outline:"none",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,letterSpacing:"0.06em"}}/>
                <button onClick={applyPromo}
                  style={{background:NAVY,border:"none",borderRadius:8,padding:"9px 14px",
                    color:WHITE,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:12,cursor:"pointer",flexShrink:0}}>APPLY</button>
              </div>
              {promoMsg&&<div style={{marginTop:6,fontSize:11,fontWeight:700,color:promoMsg.ok?GREEN:RED}}>{promoMsg.ok?"✓":"✗"} {promoMsg.text}</div>}
            </div>
            <div style={{background:WHITE,borderRadius:12,padding:"14px",boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:13,color:NAVY,marginBottom:10}}>ORDER SUMMARY</div>
              {[
                ["Subtotal",`P${cart.reduce((s,i)=>s+i.price*i.qty,0)}`],
                isMember?[`${tierLabel} discount (-${memberDisc}%)`,`-P${Math.round(cart.reduce((s,i)=>s+i.price*memberDisc/100*i.qty,0))}`]:null,
                ...
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",
                  ...
                  color:label.includes("discount")||label.includes("Promo")?GREEN:NAVY}}>
                  <span>{label}</span>
                  <span style={{fontWeight:label==="Delivery"?400:600}}>{val}</span>
                </div>
              ))}
              <div style={{display:"flex",justifyContent:"space-between",padding:"10px 0 0",
                fontSize:16,fontWeight:900,color:NAVY,fontFamily:"'Barlow Condensed',sans-serif"}}>
                <span>TOTAL</span><span>P{cartSubtotal}</span>
              </div>
              {savedTotal>0&&(
                <div style={{background:`${GREEN}18`,borderRadius:6,padding:"6px 10px",marginTop:8,
                  fontSize:11,color:GREEN,fontWeight:700,textAlign:"center"}}>
      ...
      {cart.length>0&&(
        <div style={{padding:"12px 14px",background:WHITE,borderTop:"1px solid #eee",flexShrink:0}}>
          <button onClick={()=>setShopView("checkout")}
            style={{width:"100%",padding:"16px",background:NAVY,border:"none",borderRadius:12,
              fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:17,color:WHITE,
              cursor:"pointer",minHeight:54,boxShadow:"0 4px 14px rgba(13,27,62,0.3)",
  ...
  const CheckoutView = () => {
    ...
    if(checkStep===3) return (
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
        ...
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:26,color:NAVY,marginBottom:8}}>ORDER PLACED!</div>
        <div style={{fontSize:13,color:MGRAY,lineHeight:1.7,marginBottom:24,maxWidth:300}}>
          Thank you! Your order has been received. We'll confirm once payment is verified. Delivery: 3–7 working days.
        </div>
        <div style={{background:LGRAY,borderRadius:12,padding:"14px 16px",marginBottom:20,width:"100%",maxWidth:320,textAlign:"left"}}>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:12,color:NAVY,marginBottom:8}}>ORDER REFERENCE</div>
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:18,color:GOLD2}}>{payRef.toUpperCase()}</div>
          <div style={{fontSize:11,color:MGRAY,marginTop:4}}>Keep this reference for tracking</div>
        </div>
        <button onClick={()=>{setShopView("home");setCheckStep(1);setPayMethod("");setPayRef("")}}
          style={{background:NAVY,border:"none",borderRadius:12,padding:"14px 32px",
            fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:15,color:WHITE,cursor:"pointer"}}>
          BACK TO STORE
        </button>
      </div>
    )
    ...
    return (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:NAVY,padding:"14px 16px",flexShrink:0,display:"flex",alignItems:"center",gap:12}}>
          <button onClick={()=>setShopView("cart")}
            style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,
              padding:"6px 12px",color:WHITE,fontSize:13,cursor:"pointer",
              fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,flexShrink:0}}>← BACK</button>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px",background:"#f5f6fa",WebkitOverflowScrolling:"touch"}}>
          <div style={{display:"flex",gap:6,marginBottom:16,alignItems:"center",justifyContent:"center"}}>
            {["Details","Payment","Confirm"].map((s,i)=>(
              <div key={s} style={{display:"flex",alignItems:"center",gap:6}}>
                <div style={{width:24,height:24,borderRadius:"50%",
                  background:checkStep>i+1?GREEN:checkStep===i+1?NAVY:"#ddd",
                  display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <span style={{fontSize:10,fontWeight:900,color:WHITE}}>{checkStep>i+1?"✓":i+1}</span>
                </div>
                <span style={{fontSize:10,color:checkStep===i+1?NAVY:MGRAY,fontWeight:checkStep===i+1?700:400}}>{s}</span>
            ...
            <div style={{background:WHITE,borderRadius:12,padding:"14px",boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
              ...
              {[
                ...
                <div key={f.label} style={{marginBottom:12}}>
                  <label style={{fontSize:10,fontWeight:700,color:MGRAY,fontFamily:"'Barlow Condensed',sans-serif",display:"block",marginBottom:4,letterSpacing:"0.06em"}}>{f.label}</label>
                  <input type={f.type} value={f.val} placeholder={f.ph} onChange={e=>f.set(e.target.value)}
                    style={{width:"100%",padding:"11px 12px",borderRadius:8,border:"1.5px solid #e5e7eb",fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"inherit"}}
              ...
              <div style={{background:LGRAY,borderRadius:8,padding:"10px 12px",marginBottom:12}}>
                <div style={{display:"flex",justifyContent:"space-between",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:15,color:NAVY}}>
                  <span>{cartQty} item{cartQty!==1?"s":""}</span><span>P{cartSubtotal}</span>
                </div>
                {savedTotal>0&&<div style={{fontSize:11,color:GREEN,fontWeight:700,marginTop:4}}>You save P{savedTotal}!</div>}
              </div>
              {err&&<div style={{color:RED,fontSize:12,marginBottom:10,fontWeight:600}}>{err}</div>}
              <button onClick={()=>{if(!name||!email||!phone){setErr("Please fill in all required fields.");return}setErr("");setCheckStep(2)}}
                style={{width:"100%",padding:"14px",background:NAVY,border:"none",borderRadius:10,
                  fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:15,color:WHITE,cursor:"pointer",minHeight:50}}>
                CONTINUE TO PAYMENT →
              </button>
            </div>
          )}
              ...
              <div style={{background:WHITE,borderRadius:12,padding:"14px",marginBottom:10,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
                ...
                {PAY_METHODS.map(m=>(
                  <div key={m.id} onClick={()=>setPayMethod(m.id)}
                    ...
                    <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${payMethod===m.id?NAVY:"#ddd"}`,background:payMethod===m.id?NAVY:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              {payMethod&&(
                <div style={{background:WHITE,borderRadius:12,padding:"14px",marginBottom:10,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}>
                  <div style={{background:NAVY,borderRadius:10,padding:"14px",textAlign:"center",marginBottom:14}}>
                    ...
                    {savedTotal>0&&<div style={{fontSize:11,color:GREEN,marginTop:4}}>Saved P{savedTotal}</div>}
                  </div>
                  <div style={{background:`${GOLD}18`,border:`1px solid ${GOLD}44`,borderRadius:8,padding:"10px 12px",marginBottom:12}}>
                    ...
                    {payMethod==="orange"&&<div style={{fontSize:12,color:NAVY,lineHeight:1.7}}>1. Dial <strong>*145#</strong><br/>2. Send <strong>P{cartSubtotal}</strong> to <strong>74000001</strong><br/>3. Ref: <strong>VILLAREAL-ORDER</strong></div>}
                    {payMethod==="myzaka"&&<div style={{fontSize:12,color:NAVY,lineHeight:1.7}}>1. Open MyZaka or dial <strong>*167#</strong><br/>2. Send <strong>P{cartSubtotal}</strong> to <strong>74000001</strong><br/>3. Ref: <strong>VILLAREAL-ORDER</strong></div>}
                    {payMethod==="eft"&&<div style={{fontSize:12,color:NAVY,lineHeight:1.7}}>Bank: <strong>FNB Botswana</strong><br/>Account: <strong>62012345678</strong><br/>Amount: <strong>P{cartSubtotal}</strong><br/>Ref: <strong>your email</strong></div>}
                  </div>
                  <label style={{fontSize:10,fontWeight:700,color:MGRAY,fontFamily:"'Barlow Condensed',sans-serif",display:"block",marginBottom:4,letterSpacing:"0.06em"}}>PAYMENT REFERENCE / CONFIRMATION NUMBER</label>
                  <input placeholder="e.g. TXN123456789" value={payRef} onChange={e=>setPayRef(e.target.value)}
                    style={{width:"100%",padding:"12px",borderRadius:8,border:`1.5px solid ${payRef?GOLD:"#e5e7eb"}`,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:"inherit"}}/>
                  <div style={{fontSize:10,color:MGRAY,marginTop:4}}>Enter the transaction reference from your payment confirmation</div>
                </div>
              )}
              {err&&<div style={{color:RED,fontSize:12,marginBottom:10,fontWeight:600}}>{err}</div>}
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setErr("");setCheckStep(1)}}
                  style={{flex:1,padding:"13px",background:"#f0f0f0",border:"none",borderRadius:10,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:14,color:NAVY,cursor:"pointer",minHeight:48}}>← BACK</button>
                <button onClick={confirmOrder} disabled={loading||!payMethod||!payRef}
                  style={{flex:2,padding:"13px",background:loading||!payMethod||!payRef?"#e5e7eb":GREEN,border:"none",borderRadius:10,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:14,color:loading||!payMethod||!payRef?"#aaa":WHITE,cursor:loading||!payMethod||!payRef?"not-allowed":"pointer",minHeight:48}}>
  ...
  ══════════════════════════════════════════════════════════ */
  const TicketsTab = () => (
    <div style={{overflowY:"auto",flex:1,padding:12,WebkitOverflowScrolling:"touch",background:"#f5f6fa"}}>

      {/* Member ticket discount banner */}
      {session && isMember && (
        <div style={{background:`linear-gradient(135deg,${NAVY},#1a3060)`,borderRadius:12,
          padding:"12px 16px",marginBottom:12,
          display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
              fontSize:13,color:tierColor,letterSpacing:"0.06em"}}>{tierLabel} TICKET PRICES</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:2}}>
              {isHoneyBadger ? "20% off all match tickets + free home entry" : "10% off all match tickets"}
            </div>
          </div>
          <div style={{background:tierColor,color:NAVY,fontFamily:"'Barlow Condensed',sans-serif",
            fontWeight:900,fontSize:12,padding:"5px 10px",borderRadius:6}}>
            {isHoneyBadger?"20% OFF":"10% OFF"}
          </div>
        </div>
      )}

      {/* Free home entry banner for HB */}
      {isHoneyBadger && (
        <div style={{background:`${GREEN}18`,border:`1px solid ${GREEN}44`,borderRadius:12,
          padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:18}}>🏟️</span>
          <div style={{fontSize:12,color:GREEN,fontWeight:700}}>
            Honey Badger benefit: FREE entry to all home matches!
          </div>
        </div>
      )}

      {!session && (
        <div onClick={goToAuth}
          style={{background:`linear-gradient(135deg,${GOLD},${GOLD2})`,borderRadius:12,
            padding:"12px 16px",marginBottom:12,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"space-between",
            WebkitTapHighlightColor:"transparent"}}>
          <div>
            <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:13,color:NAVY}}>🎟 LOG IN FOR MEMBER TICKET PRICES</div>
            <div style={{fontSize:11,color:"rgba(13,27,62,0.7)",marginTop:2}}>Global Fan 10% off · Honey Badger 20% off + free entry</div>
          </div>
          <div style={{background:NAVY,color:GOLD,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:11,padding:"5px 10px",borderRadius:6,flexShrink:0}}>LOG IN →</div>
        </div>
      )}
        ...
        fixtures.filter(f=>!f.result).map(fx=>(
          <div key={fx.id} style={{borderRadius:12,overflow:"hidden",marginBottom:10,
            ...
            <div style={{background:NAVY,padding:"8px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <span style={{background:fx.venue==="HOME"?GREEN:RED,color:WHITE,fontSize:9,fontWeight:900,
                padding:"2px 7px",borderRadius:4,fontFamily:"'Barlow Condensed',sans-serif"}}>{fx.venue}</span>
            </div>
            <div style={{padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
              <div style={{minWidth:0}}>
                ...
                {fx.venue==="HOME"&&isMember&&(
                  <div style={{fontSize:10,marginTop:4}}>
                    {isHoneyBadger
                      ? <span style={{color:GREEN,fontWeight:700}}>🏟️ FREE for Honey Badger members</span>
                      : <span style={{color:tierColor,fontWeight:700}}>
                          P{ticketPrice(25)} <span style={{color:MGRAY,fontDecoration:"line-through",fontWeight:400}}>P25</span>
                        </span>
                    }
                  </div>
                )}
              ...
              <button style={{
                background:isHoneyBadger&&fx.venue==="HOME"?GREEN:GOLD,
                border:"none",borderRadius:8,padding:"9px 16px",
                fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:13,
                color:NAVY,cursor:"pointer",flexShrink:0,minHeight:40}}>
                {fx.venue==="HOME"
                  ? isHoneyBadger
                    ? "FREE ENTRY"
                    : `BUY P${ticketPrice(25)}`
                  : "AWAY"}
              </button>
            </div>
          </div>
  ...
  /* ══════════════════════════════════════════════════════════
     MEMBERSHIP TAB — matches For You page tiers exactly
  ══════════════════════════════════════════════════════════ */
  const MembershipTab = () => {
    ...
    const TIERS = [
      ...
      },
      {
        ...
        emoji:"🦡",
        price:{ yearly:500, monthly:50 },
        youthPrice:null,
        saveBadge:"Save P100",
        ageRestriction:"18+ ONLY",
        highlight:false,
        bg:`linear-gradient(135deg,${GOLD}22,${GOLD2}11)`,
        border:GOLD,
        accentColor:GOLD,
        perks:[
    ...
    return (
      <div style={{overflowY:"auto",flex:1,WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{background:`linear-gradient(160deg,${NAVY},#0a1428)`,
          padding:"clamp(18px,5vw,28px) clamp(14px,4vw,20px) 20px",
            ...
            <div style={{background:"rgba(255,255,255,0.08)",borderRadius:10,
              ...
              <div>
                ...
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                  ...
                  {(profile?.full_name||session.user.email?.split("@")[0]||"FAN").toUpperCase()}
                </div>
              </div>
              <div style={{background:tierColor,color:NAVY,fontFamily:"'Barlow Condensed',sans-serif",
                fontWeight:900,fontSize:11,padding:"5px 12px",borderRadius:20,flexShrink:0}}>
                {tierLabel}
              </div>
            </div>
          )}
          ...
          <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
            fontSize:"clamp(24px,7vw,36px)",color:WHITE,lineHeight:1}}>
            MEMBERSHIP PLANS
          </div>
          ...
          {/* Billing toggle */}
          <div style={{display:"flex",background:"rgba(255,255,255,0.1)",borderRadius:10,padding:3,marginBottom:4}}>
        ...
        {/* Tier cards */}
        <div style={{background:"#f5f6fa",padding:"12px 12px 24px"}}>
          {TIERS.map(t=>{
            const isCurrent = tier === t.id
            const price = t.price[billing]
            return (
              <div key={t.id} style={{background:WHITE,borderRadius:16,marginBottom:12,
                overflow:"hidden",boxShadow:t.highlight?"0 4px 20px rgba(245,197,24,0.2)":"0 1px 8px rgba(0,0,0,0.07)",
                border:`2px solid ${isCurrent?GREEN:t.highlight?t.border:"#eee"}`,
                position:"relative"}}>
                ...
                {isCurrent&&(
                  <div style={{position:"absolute",top:0,left:0,
                    background:GREEN,color:WHITE,fontSize:9,fontWeight:900,
                    padding:"4px 12px",borderRadius:"14px 0 10px 0",
                    fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.08em"}}>
                    ✓ YOUR PLAN
                  </div>
                )}
                ...
                {/* Tier header */}
                <div style={{background:t.id==="honey_badger"?`linear-gradient(135deg,${NAVY},#1a3060)`:NAVY,
                  padding:"16px 16px 12px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,marginTop:isCurrent||t.popularBadge?16:0}}>
                    <div style={{width:42,height:42,borderRadius:10,
                      background:`${t.accentColor}22`,border:`1.5px solid ${t.accentColor}44`,
                      display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>
                      {t.emoji}
                    </div>
                    <div>
                      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                        fontSize:16,color:t.accentColor,letterSpacing:"0.08em"}}>{t.label}</div>
                      <div style={{fontSize:11,color:"rgba(255,255,255,0.55)",marginTop:1}}>{t.sublabel}</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{padding:"14px 16px"}}>
                  <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:4}}>
                    <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                      fontSize:price===0?"clamp(20px,6vw,26px)":"clamp(28px,8vw,36px)",color:NAVY}}>
                      {price===0?"FREE":`P${price}`}
                    </span>
                    {price>0&&<span style={{fontSize:12,color:MGRAY}}>/{billing==="yearly"?"yr":"mo"}</span>}
                    {t.saveBadge&&isYearly&&(
                      <span style={{background:`${GREEN}22`,color:GREEN,fontSize:10,fontWeight:800,
                        padding:"2px 7px",borderRadius:4,fontFamily:"'Barlow Condensed',sans-serif"}}>
                        {t.saveBadge}
                      </span>
                    )}
                    {t.ageRestriction&&(
                      <span style={{background:`${RED}18`,color:RED,fontSize:9,fontWeight:800,
                        padding:"2px 7px",borderRadius:4,fontFamily:"'Barlow Condensed',sans-serif",marginLeft:"auto"}}>
                        {t.ageRestriction}
                      </span>
                    )}
                  </div>
                  {t.youthPrice&&isYearly&&(
                    <div style={{fontSize:11,color:MGRAY,marginBottom:10}}>
                      Youth: P{t.youthPrice.yearly}/yr
                    </div>
                  )}
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:14,marginTop:10}}>
                    {t.perks.map((perk,i)=>(
                      <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                        <span style={{color:t.accentColor,fontSize:13,flexShrink:0}}>✓</span>
                        <span style={{fontSize:12,color:NAVY,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600}}>{perk}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {/* CTA button */}
                  {isCurrent ? (
                    <div style={{width:"100%",padding:"12px",background:`${GREEN}18`,
                      border:`1.5px solid ${GREEN}`,borderRadius:10,textAlign:"center",
                      fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                      fontSize:14,color:GREEN}}>
                      ✓ CURRENT PLAN
                    </div>
                  ) : t.id==="premium_free" ? (
                    <div style={{width:"100%",padding:"12px",background:"#f8f9fa",
                      border:"1.5px solid #eee",borderRadius:10,textAlign:"center",
                      fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,
                      fontSize:13,color:MGRAY}}>
                      FREE — No signup needed
                    </div>
                  ) : (
                    <button onClick={session ? openMembership : goToAuth}
                      style={{width:"100%",padding:"13px",minHeight:48,
                        background:t.id==="honey_badger"?GOLD:NAVY,
                        border:"none",borderRadius:10,cursor:"pointer",
                        fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,
                        fontSize:15,color:t.id==="honey_badger"?NAVY:WHITE,
                        WebkitTapHighlightColor:"transparent",
                        boxShadow:t.id==="honey_badger"?"0 4px 14px rgba(245,197,24,0.4)":"0 2px 8px rgba(13,27,62,0.2)",
                        letterSpacing:"0.04em"}}>
                      {session ? `JOIN ${t.label} →` : `LOG IN TO JOIN →`}
                    </button>
                  )}
                </div>
              </div>
  ...
  ══════════════════════════════════════════════════════════ */
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",background:WHITE,overflow:"hidden",position:"relative"}}>
      {/* Tab bar */}
      {(["home","collection","product","customize","tickets","membership"].includes(shopView))&&(
        <div style={{display:"flex",borderBottom:`1px solid #eee`,padding:"0 14px",gap:14,
          overflowX:"auto",flexShrink:0,alignItems:"center",background:WHITE,zIndex:10}}>
          ...
          <button onClick={()=>setShopView("cart")}
            style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",
              position:"relative",padding:"8px 0",flexShrink:0,WebkitTapHighlightColor:"transparent"}}>
